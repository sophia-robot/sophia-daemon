class Sophia:
  ROS_CHAN_REF_POS='/ref/pos'
  ROS_CHAN_CMD='/cmd'
  ROS_CHAN_STATE_POS='/state/pos'
  ROS_CHAN_STATE_TORQUE='/state/torque'
  ROS_CHAN_STATE_ACC='/state/acc'
  ROS_CHAN_STATE_GYRO='/state/gyro'
  ROS_CHAN_HEART='/heart/beat'
  ROS_CHAN_WALKING='/ref/walking'
  ROS_CHAN_TURN_CTRL='/cmd/turn'
  ROS_CHAN_WALKING_K='/gains/walking/k'
  init_name = "none"
  sophia_name = "none"

  NAME = 0

  ENABLE_RIGHT = True
  ENABLE_LEFT  = True

  ENABLE_FOOT_RIGHT = True
  ENABLE_FOOT_LEFT  = True

  ENUM_NAME          = 0
  ENUM_TORQUE_EN_0   = 1
  ENUM_TORQUE_EN_1   = 2
  ENUM_REF           = 3
  ENUM_STATE         = 4
  ENUM_ENABLED       = 5
  ENUM_POS_MAX       = 6
  ENUM_POS_MIN       = 7
  ENUM_MOT_INDEX     = 8
  ENUM_POS_OFFSET    = 9

  

  OK = False
  FAIL = True

  #        name   torque_en_0  torque_en_1  pos_id  enc_id  enabled         max_pos (rad)   min_pos (rad)  filter_id  offset (deg)
  IDs = { ("lhy", 0x30,        0x31,        0x3c,   0x30,   ENABLE_LEFT ,        1.0,            -1.0,                  0,   -0.4     ),
          ("lhr", 0x32,        0x33,        0x3d,   0x32,   ENABLE_LEFT ,        0.2,            -0.2,                  1,    0.0     ),
          ("lhp", 0x34,        0x35,        0x3e,   0x34,   ENABLE_LEFT ,        0.7,            -1.3,                  2,    5.0     ),
          ("lkp", 0x36,        0x37,        0x3f,   0x36,   ENABLE_LEFT ,        1.2,             0.0,                  3,    0.0     ),
          ("lay", 0x38,        0x38,        0x38,   0x38,   ENABLE_LEFT ,        1.0,            -1.0,                  4,    0.0     ),
          ("lar", 0x39,        0x39,        0x39,   0x39,   ENABLE_LEFT ,        1.0,            -1.0,                  5,    0.0     ),
          ("lap", 0x3a,        0x3a,        0x3a,   0x3a,   ENABLE_FOOT_LEFT,    0.4,            -1.0,                  6,   80.0     ),
          ("ltp", 0x3b,        0x3b,        0x3b,   0x3b,   False        ,       1.0,            -1.0,                  7,    0.0     ),
          ("rhy", 0x10,        0x11,        0x1c,   0x10,   ENABLE_RIGHT,        1.0,            -1.0,                  8,  -20.4     ),
          ("rhr", 0x12,        0x13,        0x1d,   0x12,   ENABLE_RIGHT,        0.2,            -0.2,                  9,    0.0     ),
          ("rhp", 0x14,        0x15,        0x1e,   0x14,   ENABLE_RIGHT,        0.7,            -1.3,                 10,    5.0     ),
          ("rkp", 0x16,        0x17,        0x1f,   0x16,   ENABLE_RIGHT,        1.2,             0.0,                 11,    0.0     ),
          ("ray", 0x18,        0x18,        0x18,   0x18,   ENABLE_RIGHT,        1.0,            -1.0,                 12,    0.0     ),
          ("rar", 0x19,        0x19,        0x19,   0x19,   ENABLE_RIGHT,        1.0,            -1.0,                 13,    0.0     ),
          ("rap", 0x1a,        0x1a,        0x1a,   0x1a,   ENABLE_FOOT_RIGHT,   0.4,            -1.0,                 14,   80.0     ),
          ("rtp", 0x1b,        0x1b,        0x1b,   0x1b,   False       ,        1.0,            -1.0,                 15,    0.0     )
        }




  def Sophia(self):
    self.sophia_name = "sophia_name"
    print("made the class")

  def __init__(self):
    print("in init")
    self.init_name = "init_name"

  def printThis(self):
    print("this")
    print(self.init_name)
    print(self.sophia_name)

